...
public sealed class EchoForm : Form {
 ...
 public EchoForm() { Text="Echo – Rewrite in My Voice"; Width=660; Height=460; var menu=new MenuStrip(); var file=new ToolStripMenuItem("&File"); file.DropDownItems.Add("E&xit",null,(_,_)=>Application.Exit()); menu.Items.Add(file); MainMenuStrip=menu; Controls.Add(samples); Controls.Add(finish); Controls.Add(status); Controls.Add(menu); finish.Click+=async (_,_)=>await Onboard(); Load+=(_,_)=> { (consent,profile)=store.Read(); Render(); RegisterHotKey(Handle,HOTKEY,3,(uint)Keys.E); }; FormClosing+=(s,e)=> { if(e.CloseReason==CloseReason.UserClosing){e.Cancel=true;Hide();} }; }
 ...
 [DllImport("user32.dll")] static extern short GetAsyncKeyState(int vKey); [DllImport("user32.dll")] static extern uint GetClipboardSequenceNumber();
